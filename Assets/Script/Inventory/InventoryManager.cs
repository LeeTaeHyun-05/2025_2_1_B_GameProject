...
public class InventoryManager : MonoBehaviour
{
    ...
    public void ToggleInventory()               //인벤 UI를 열거나 닫는 함수
    {
        ...
        if (isInventoryOpen)
        {
            Cursor.lockState = CursorLockMode.None;     //인벤 열면 커서 보이기
            Cursor.visible = true;
        }
        else
        {
            Cursor.lockState = CursorLockMode.Locked;   //인벤 닫으면 커서 숨기기
    ...
    {
        foreach (InventorySlot slot in slots)           //1단계 : 이미 있는 아이템에 추가 시도
        {
            if (slot.item == item && slot.amount < item.maxStack)   //같은 아이템이고 최대 스택보다 작으면
            {
                ...
                amount -= amountToAdd;                              //기존 추가한 개수에 남은 개수를 구함
    ...
    public void RemoveItem (ItemData item, int amount = 1)    //아이템을 인벤에서 제거하는 함수
    {
